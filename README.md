...
긴 텍스트를 입력받아, 주어진 내용만으로 답할 수 있는 퀴즈 질문을 정확히 하나 생성하도록
베이스 언어 모델을 Unsloth QLoRA로 파인튜닝하는 프로젝트입니다. 본 실험에서는 영어
Wikipedia 지문으로 구성된 SQuAD의 `context`를 원천 데이터로 사용합니다.

> 현재 상태: 데이터 파이프라인 구축 완료, Unsloth Studio 실제 학습 및 결과 기록 예정

## 1. 프로젝트 소개

긴 텍스트를 바탕으로 평가·복습용 질문을 직접 작성하는 데에는 많은 시간이 필요합니다.
본 프로젝트는 교사, 학습자, 교육 콘텐츠 제작자가 텍스트를 입력하면 해당 내용만으로 답할 수
...
4. 자연스럽고 문법적으로 올바른 질문을 생성한다.
5. 정답, 선택지, 설명 등 불필요한 내용을 출력하지 않는다.

## 3. 주요 기능

- 긴 텍스트를 입력받아 퀴즈 질문을 정확히 하나 생성
...
Data Recipes 실행에서는 API rate limit 이후 437개의 생성 결과를 확보했습니다. 자동 품질 검사에서
중복 context, 중복 질문, 형식 오류를 확인한 결과 437개가 모두 기본 검사 기준을 통과했습니다.
최종 분할은 train 349개, validation 43개, test 45개입니다.
...
| Environment | Unsloth Studio on Google Colab T4 GPU |
| Data Source | SQuAD context |
| Language | Python |

## 6. 설치 및 실행 방법

### Track A: Unsloth Studio (본 프로젝트의 실제 수행 방법)

1. [공식 Unsloth Studio Colab](https://colab.research.google.com/github/unslothai/notebooks/blob/main/nb/Unsloth_Studio.ipynb)을 엽니다.
2. Colab의 T4 GPU 런타임에서 `Run all`을 실행하고 Studio를 시작합니다.
3. Studio에서 베이스 모델을 로드합니다.
4. `data/final/train.jsonl`과 `validation.jsonl`을 import합니다.
5. Data Recipes 미리보기에서 role 태깅과 chat 형식을 확인합니다.
6. QLoRA 설정으로 실제 학습하고 loss와 GPU 사용률을 기록합니다.
7. Model Arena에서 베이스 모델과 파인튜닝 모델을 비교합니다.
8. LoRA adapter와 GGUF를 export합니다.

세부 실행 절차는 [`docs/STUDIO_WORKFLOW.md`](docs/STUDIO_WORKFLOW.md)를 참고하십시오.
...
| 항목 | 초기 설정 | 선택 근거 |
|---|---|---|
| 베이스 모델 | `unsloth/Qwen2.5-3B-Instruct` 후보 | 다국어 지시 수행 능력과 Colab GPU 환경의 VRAM 균형 |
...
실제 Unsloth Studio QLoRA 학습 후 다음 자료를 추가합니다.
...
학습 완료 후 [`images/`](images/)에 다음 실행 결과를 추가합니다.
